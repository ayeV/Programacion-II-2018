...
namespace SP
{
    ...
    public partial class SegundoParcial : Form
    {
        ...
        private static string ObtenerListadoFrutas()
        {
            ...
            while (lector.Read())
            {
                sb.AppendLine(string.Format("{0}-{1}-{2}-{3}", lector[0], lector[1], lector[2], lector[3]));
            }
            conexion.Close();
        ...
        private static bool AgregarFrutas()
        {
            ...
            SqlCommand comando = new SqlCommand();
            // comando.CommandText = string.Format("INSERT INTO sp_lab_II values ({0},'{1}',{2},{3})",);
            comando.CommandType = System.Data.CommandType.Text;
            comando.Connection = conexion;
